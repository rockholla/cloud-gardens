...
module.exports = function(profile, region) {
    ...
    this.terraform = function(action, stateBucket, variables, module) {
        ...
        return new Promise(function(resolve, reject) {
            try {
                fs.unlinkSync(path.join('terraform', 'aws', module, '.terraform', 'terraform.tfstate'));
            } catch (error) {}
            try {
                var force = (action == 'destroy' ? ' -force' : '');
                shell.execSync(
                    `terraform init -get=true -backend=true -backend-config="bucket=${stateBucket}" -backend-config="key=${stateKey}" -backend-config="region=${_this.region}"`,
                    { cwd: 'terraform/aws/' + module, stdio: ['pipe', 'pipe', process.stderr] }
                );
                var cmd = `terraform ${action} -var profile="${_this.profile}" -var region="${_this.region}"`;
                for (var item in variables) {
                    cmd += ` -var ${item}="${variables[item]}"`
                }
                if (action == 'destroy') cmd += ' -force';
                shell.execSync(cmd, { cwd: 'terraform/aws/' + module, stdio: [0, 1, 2] });
                resolve(true);
            } catch(error) {
                reject(error);
            }
